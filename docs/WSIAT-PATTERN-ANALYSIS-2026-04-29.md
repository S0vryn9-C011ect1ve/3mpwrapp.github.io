# WSIAT Pattern Analysis Report
## 99,036 Decisions Analyzed (1987-2026)

**Generated:** 2026-04-30  
**Total Decisions:** 99,036  
**Year Range:** 1987-2026 (40 years)

---

## Executive Summary

This report analyzes 99,036 WSIAT decisions spanning 40 years (1987-2026) to identify patterns in:
- Legal issues and keywords
- Decision types and procedural matters
- Temporal trends and workload evolution
- Representative participation

---

## 1. Most Common Legal Issues

| Rank | Keyword | Count | % of Decisions |
|------|---------|-------|----------------|
| 1 | NEL | 20,680 | 20.88% |
| 2 | permanent impairment | 11,841 | 11.96% |
| 3 | LOE | 10,838 | 10.94% |
| 4 | loss of earnings | 9,217 | 9.31% |
| 5 | FEL | 7,120 | 7.19% |
| 6 | chronic pain | 6,876 | 6.94% |
| 7 | reconsideration | 6,153 | 6.21% |
| 8 | SIEF | 4,654 | 4.70% |
| 9 | right to sue | 1,763 | 1.78% |
| 10 | initial entitlement | 1,250 | 1.26% |
| 11 | pre-existing | 1,075 | 1.09% |
| 12 | vocational rehabilitation | 235 | 0.24% |
| 13 | PTSD | 159 | 0.16% |
| 14 | CPD | 120 | 0.12% |
| 15 | Section 31 | 42 | 0.04% |

### Top 3 Insights:

1. **NEL** appears in 20.88% of decisions (20,680 cases)
2. **permanent impairment** appears in 11.96% of decisions (11,841 cases)
3. **LOE** appears in 10.94% of decisions (10,838 cases)

---

## 2. Decision Type Distribution

| Decision Type | Count | % of Total |
|---------------|-------|------------|
| Standard Decision | 99,036 | 100.00% |
| Reconsideration | 0 | 0.00% |
| Interim Decision | 0 | 0.00% |
| Leave to Appeal | 0 | 0.00% |
| Extension of Time | 0 | 0.00% |
| Right to Sue Application | 0 | 0.00% |

### Key Findings:

- **Standard Decisions** make up 100.00% of all cases
- **Reconsiderations** account for 0.00% (0 cases)
- **Right to Sue Applications** represent 0.00% of decisions

---

## 3. Temporal Trends: 40 Years of WSIAT

### Peak Decision Years (Top 10)

| Rank | Year | Decisions |
|------|------|-----------|
| 1 | 00 | 4,502 |
| 2 | 17 | 4,248 |
| 3 | 18 | 3,969 |
| 4 | 01 | 3,844 |
| 5 | 16 | 3,633 |
| 6 | 07 | 3,445 |
| 7 | 08 | 3,154 |
| 8 | 15 | 3,084 |
| 9 | 06 | 3,046 |
| 10 | 03 | 3,007 |

### Decade-by-Decade Breakdown

| Decade | Total Decisions |
|--------|-----------------|
| 0s | 31,928 |
| 10s | 31,691 |
| 20s | 10,772 |
| 80s | 3,805 |
| 90s | 16,403 |
| 2020s | 44 |

### Insights:

- Peak year: **00** with 4,502 decisions
- Average decisions per year: 2,476
- Busiest decade: 0s

---

## 4. Representative Participation

### Vice-Chair Workload (Top 20)

| Rank | Vice-Chair | Decisions |
|------|------------|-----------|
| 1 | R. Nairn | 3,860 |
| 2 | M. Keil | 3,605 |
| 3 | J. Moore | 2,981 |
| 4 | V. Marafioti | 2,484 |
| 5 | S. Ryan | 2,400 |
| 6 | S. Sutherland | 2,367 |
| 7 | J. Josefo | 2,213 |
| 8 | M. Crystal | 2,023 |
| 9 | U. Ferdinand | 1,931 |
| 10 | S. Peckover | 1,902 |
| 11 | E. Smith | 1,829 |
| 12 | J. Bigras | 1,747 |
| 13 | E. Newman | 1,722 |
| 14 | B. Cook | 1,630 |
| 15 | L. Gehrke | 1,630 |
| 16 | A. Baker | 1,606 |
| 17 | A. Signoroni | 1,582 |
| 18 | S. Martel | 1,533 |
| 19 | A. Patterson | 1,413 |
| 20 | B. Kalvin | 1,337 |

### Statistics:

- **Total Vice-Chairs:** 3260
- **Decisions with Vice-Chair Data:** 99,036 (100.0%)
- **Worker Side Members:** 3115 unique individuals
- **Employer Side Members:** 3152 unique individuals

---

## 5. Data Quality Assessment

| Metric | Value |
|--------|-------|
| Total Decisions | 99,036 |
| Decisions with Keywords | 82,111 |
| Decisions with Summaries | (To be calculated) |
| Decisions with Vice-Chair Data | 99,036 |
| Decisions with Year Data | 94643 |

**Coverage:** 95.6% of decisions have complete year data

---

## 6. Cross-Provincial Comparison

| Metric | Ontario WSIAT | BC WCAT | Ratio |
|--------|---------------|---------|-------|
| **Total Decisions** | 99,036 | 7,386 | 13.4:1 |
| **Year Coverage** | 1987-2026 (40 years) | 2020-2026 (6 years) | 6.7x |
| **Keywords/Metadata** | ✅ Full | ❌ None | ∞ |
| **Open Data** | ✅ CSV Export | ❌ None | ∞ |
| **Searchable** | ✅ Advanced | ⚠️ Basic | Advanced |

**Ontario provides 13.4x more decisions with 100% metadata transparency.**

---

## 7. Recommendations for Knowledge Base & Templates

### High-Priority Knowledge Base Articles

Based on keyword frequency:

1. **NEL** - 20,680 cases (20.88%)
2. **permanent impairment** - 11,841 cases (11.96%)
3. **LOE** - 10,838 cases (10.94%)
4. **loss of earnings** - 9,217 cases (9.31%)
5. **FEL** - 7,120 cases (7.19%)

### Template Priorities

1. **Reconsideration Request Template** (0 cases)
2. **Right to Sue Application Template** (0 cases)
3. **Extension of Time Request Template** (0 cases)

---

## 8. Visualization Opportunities

### Recommended Visualizations:

1. **Keyword Trend Chart** - Line graph showing evolution of top 10 keywords over 40 years
2. **Decision Type Pie Chart** - Breakdown of standard vs. procedural decisions
3. **Annual Volume Bar Chart** - Decisions per year (1987-2026)
4. **Vice-Chair Workload Heatmap** - Distribution of decisions across top vice-chairs
5. **Decade Comparison** - Side-by-side comparison of 1980s vs 2020s patterns

---

## Data Access

- **Full Dataset:** [/data/tribunal-decisions/wsiat/decisions-by-year/](../data/tribunal-decisions/wsiat/decisions-by-year/)
- **Metadata:** [/data/tribunal-decisions/wsiat/wsiat-metadata.json](../data/tribunal-decisions/wsiat/wsiat-metadata.json)
- **Analysis Results:** [/data/tribunal-decisions/wsiat/wsiat-analysis-patterns.json](../data/tribunal-decisions/wsiat/wsiat-analysis-patterns.json)

---

**Generated by:** WSIAT Pattern Analysis Script v1.0  
**Source:** Official WSIAT CSV Export  
**License:** Open Data (CC BY 4.0)
