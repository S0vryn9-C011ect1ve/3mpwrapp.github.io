#!/usr/bin/env node
/**
 * 🔬 COMPREHENSIVE BC IGSRC SCRAPER (2020-2026)
 * British Columbia Income and Disability Assistance Appeal Tribunal
 * 
 * CRITICAL DATASET: BC disability benefit appeal decisions
 * Focus: ODSP-equivalent system in BC, denial patterns, eligibility disputes
 * Compare to: Ontario ONSBT (social benefits tribunal)
 * 
 * Output: Year-by-year JSON files with enhanced metadata
 * bcigsrc-2020-complete.json
 * bcigsrc-2021-complete.json
 * ... through 2026
 * 
...
async function fetchYearCaseList(year) {
  console.log(`\n📅 Fetching BC IGSRC case list for ${year}...`);
  ...
  while (true) {
    ...
    const url = `${CANLII_BASE}/caseBrowse/en/bcigsrc/?${params.toString()}`;
...
async function fetchCaseDetails(caseId, year) {
  const url = `${CANLII_BASE}/caseBrowse/en/bcigsrc/${caseId}?api_key=${CANLII_API_KEY}`;
...
function extractKeywords(html) {
  ...
  const keywordGroups = {
    issues: [
      'disability assistance', 'income assistance', 'pwd', 'ppmb',
      'medical condition', 'disability designation', 'eligibility',
      'reconsideration', 'appeal', 'benefit reduction', 'benefit termination',
      'hardship assistance', 'crisis grant', 'shelter allowance'
    ],
    outcomes: [
      'dismissed', 'allowed', 'granted', 'denied', 'confirmed',
      'overturned', 'varied', 'remitted', 'withdrawn', 'settled'
    ],
    legal_tests: [
      'employment and assistance act', 'eaa', 'disability designation',
      'prescribed professional', 'substantial restriction', 'daily living activities',
      'substantially restrict', 'continuous'
    ],
    remedies: [
      'benefit restored', 'retroactive payment', 'eligibility confirmed',
      'designation granted', 'assistance approved'
    ]
  };
...
function extractOutcomeFromKeywords(keywords) {
  ...
  if (keywordText.includes('appeal dismissed') || keywordText.includes('ministry decision confirmed')) {
    return 'Dismissed - Ministry Upheld';
  }
  if (keywordText.includes('appeal allowed') || keywordText.includes('ministry decision overturned') ||
      keywordText.includes('eligibility granted')) {
    return 'Allowed - Ministry Overturned';
  }
...
function extractOutcome(caseData, html) {
  ...
  // Patterns for BC IGSRC decisions
  ...
  if (text.includes('ministry decision is confirmed') || text.includes('ministry decision confirmed')) {
    return 'Dismissed - Ministry Upheld';
  }
  if (text.includes('ministry decision is rescinded') || text.includes('ministry decision rescinded')) {
    return 'Allowed - Ministry Overturned';
  }
  if (text.includes('remitted') || text.includes('referred back')) {
    return 'Remitted to Ministry';
  }
...
function extractLegislationFromKeywords(keywords) {
  ...
  // Pattern: "Employment and Assistance Act, s. X" or "EAA, s. X(Y)" etc.
  const patterns = [
    /Employment and Assistance Act,\s*[Ss]\.\s*[\d]+(\([\d]+\))?/gi,
    /EAA,\s*[Ss]\.\s*[\d]+(\([\d]+\))?/gi,
    /Employment and Assistance Regulation,\s*[Ss]\.\s*[\d]+/gi,
    /B\.C\. Reg\. [\d]+\/[\d]+/gi,
    /Disability Assistance Act/gi
  ];
  ...
  patterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        // Normalize "EAA" to full name
        let normalized = match;
        if (match.startsWith('EAA,')) {
          normalized = 'Employment and Assistance Act, ' + match.substring(5);
        }
...
function parseDecision(caseData, fullData) {
  ...
  return {
    ...
    // Metadata
    tribunal: "British Columbia Income and Disability Assistance Appeal Tribunal",
    database: "bcigsrc",
...
async function scrapeYear(year) {
  console.log('\n' + '='.repeat(70));
  console.log(`📊 SCRAPING BC IGSRC DECISIONS FOR ${year}`);
  console.log('='.repeat(70));
  
  const outputFile = path.join(OUTPUT_DIR, `bcigsrc-${year}-complete.json`);
  const progressFile = path.join(OUTPUT_DIR, `.progress-bcigsrc-${year}.json`);
  ...
  for (let i = 0; i < caseList.length; i++) {
    ...
    try {
      ...
      if (error.message === 'QUOTA_EXCEEDED') {
        ...
        const partialFile = path.join(OUTPUT_DIR, `bcigsrc-${year}-PARTIAL.json`);
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════════╗');
  console.log('║  🔬 COMPREHENSIVE BC IGSRC SCRAPER (2020-2026)                    ║');
  console.log('║  Income & Disability Assistance Appeals - BC Social Benefits     ║');
  console.log('╚════════════════════════════════════════════════════════════════════╝');
  ...
  for (const year of YEARS) {
    try {
      ...
      summary.files.push(`bcigsrc-${year}-complete.json`);
  ...
  // Save summary
  const summaryFile = path.join(OUTPUT_DIR, 'bcigsrc-scraping-summary.json');
