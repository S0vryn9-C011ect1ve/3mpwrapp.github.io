#!/usr/bin/env node
/**
 * 🔬 COMPREHENSIVE ONSBT SCRAPER (2020-2026)
 * Ontario Social Benefits Tribunal - Full Dataset Collection
 * 
 * CRITICAL DATASET: ODSP disability benefits appeals
 * Focus: Why people get denied ODSP, medical evidence standards, disability definition
 * 
 * Output: Year-by-year JSON files with enhanced metadata
 * onsbt-2020-complete.json
...
async function fetchYearCaseList(year) {
  ...
  while (true) {
    ...
    const url = `${CANLII_BASE}/caseBrowse/en/onsbt/?${params}`;
...
async function fetchFullDecision(caseId) {
  ...
  const url = `${CANLII_BASE}/caseBrowse/en/onsbt/${caseId}/?${params}`;
...
function extractLegislationFromKeywords(keywords) {
  ...
  const patterns = [
    ...
    /ODSPA,\s*[\d]{4},\s*s\.\s*[\d]+/gi,
    /ODSPA,\s*s\.\s*[\d]+/gi,
    /Ontario Works Act,\s*[\d]{4},\s*s\.\s*[\d]+/gi,
    /OWA,\s*s\.\s*[\d]+/gi,
    /Social Assistance Act,\s*s\.\s*[\d]+/gi,
...
async function scrapeYear(year) {
  console.log('\n' + '='.repeat(70));
  console.log(`📊 SCRAPING ONSBT DECISIONS FOR ${year}`);
  console.log('='.repeat(70));
  
  const outputFile = path.join(OUTPUT_DIR, `onsbt-${year}-complete.json`);
  const progressFile = path.join(OUTPUT_DIR, `.progress-onsbt-${year}.json`);
  ...
  for (let i = 0; i < caseList.length; i++) {
    ...
    try {
      ...
      if (error.message === 'QUOTA_EXCEEDED') {
        ...
        console.log('\n💡 Alternative: Start a different tribunal (ONWSIB/HRTO)');
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════════════╗');
  console.log('║  🔬 COMPREHENSIVE ONSBT SCRAPER (2020-2026)                       ║');
  console.log('║  Ontario Social Benefits Tribunal - ODSP Appeals                   ║');
  console.log('╚════════════════════════════════════════════════════════════════════╝');
  console.log();
