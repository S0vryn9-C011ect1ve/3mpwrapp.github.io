...
console.log(`\nONSBT Keyword Analysis (${allCases.length} cases)\n${'='.repeat(60)}`);
...
sorted.forEach(([k, v], idx) => {
  ...
  console.log(`${(idx + 1).toString().padStart(2)}. ${k.padEnd(45)} ${v.toString().padStart(5)} (${percent.padStart(5)}%)`);
});
...
Object.entries(patterns)
  ...
  .forEach(([pattern, count]) => {
    ...
    console.log(`${pattern.padEnd(30)} ${count.toString().padStart(5)} (${percent.padStart(5)}%)`);
  });
